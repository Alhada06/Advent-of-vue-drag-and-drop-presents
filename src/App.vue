...
<template>
  <div class="flex flex-col items-center mt-24 min-h-screen w-full">
    ...
    <ChristmasTree @drop="onDrop" @dragover.prevent @dragenter.prevent :presents="underTheTree" class="mt-16" />
    <div class="pt-32 mt-32 bg-gray-100 w-full justify-center flex-1">
      <div class="flex items-end justify-center" v-auto-animate>
        ...
        <ChristmasPresent draggable="true" @dragstart="startDrag($event,index)" v-for="(p, index) in presents" :key="p"
          :name="p" />
      </div>
    </div>
  </div>
